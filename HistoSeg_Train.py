...
train_images_arg = args.train_images
train_masks_arg = args.train_masks
val_images_arg = args.val_images
val_masks_arg = args.val_masks
...
print("Train Images Path: " + train_images_arg)
print("Train Masks Path: " + train_masks_arg)
print("Val Images Path: " + val_images_arg)
print("Val Masks Path: " + val_masks_arg)
print("Height: " + str(height_arg))
print("Width: " + str(width_arg))
print("Epochs: " + str(epochs_arg))
print("Batch Size: " + str(batch_arg))
print("-------------------------------------")
...
X_train = np.load(train_images_arg)
y_train = np.load(train_masks_arg)

X_test = np.load(val_images_arg)
y_test = np.load(val_masks_arg)
...
def q_au(x1,x2,qau_n):
  f_a = x1
  f_b = Conv2D(1, (1, 1), padding='same',name='Quick_AU'+str(qau_n), use_bias=False)(x2)
  f_act = tf.keras.layers.Activation('sigmoid')(f_b)
  f_add = tf.keras.layers.Add()([f_act, f_a])
  return f_add
...
def HistoSeg(weights='pascal_voc', input_tensor=None, input_shape=(512, 512, 3), classes=21, backbone='mobilenetv2',
    ...
    if backbone == 'xception':
        ...
        x = _inverted_res_block(x_1, filters=64, alpha=alpha, stride=1,  # 1!
    ...
    x = q_au(x,x,2)
    x_1 = Conv2D(256, (1, 1), padding='same',use_bias=False, name='q_au_conv')(x_1)
    z = Add()([x, x_1])


    
    x = Activation(tf.nn.relu)(z)
    x = Dropout(0.1)(x)
    # HistoSeg decoder
    ...
    model = Model(inputs, x, name='HistoSegplus')
...
plot_model(model, to_file='HistoSeg_model_plot.png', show_shapes=True, show_layer_names=True)

callbacks = [
    EarlyStopping(patience=10,   verbose=1),
    #ReduceLROnPlateau(factor=0.1,  patience=3, min_lr=0.00001, verbose=1),
    ModelCheckpoint('HistoSeg_epoch:{epoch:02d}_iou:{val_mean_io_u:.4f}_acc:{val_accuracy:.4f}_f1:{val_f1_m:.4f}_dice:{val_dice_coef:.4f}_.h5', monitor='val_accuracy',  save_best_only=False, save_weights_only=True, verbose=1)
]
